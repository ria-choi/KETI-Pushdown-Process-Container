...
int main(int argc, char const *argv[])
{
	...
	snippetmanager.InitSnippetManager(tblManager);
	//init WorkID
	// WorkID=0;
...
Status SnippetSampleServiceImpl::SetSnippet(ServerContext* context,
    ...
    //여기 매니저로 보내는거
    snippetmanager.NewQuery(snippetqueue,bufma);
    return Status::OK;
  }
...
void AppendProjection(SnippetStruct &snippetdata,Value &Projectiondata){
	for(int i = 0; i < Projectiondata.Size(); i++){
		...
		for(int j = 0; j < Projectiondata[i].Size(); j++){
			Projection tmpPro;
			if(j == 0){
				tmpPro.type = 0;
				tmpPro.value = Projectiondata[i][j].GetString();
			}else{
